...
5. **Variety & intra-session balance**: do NOT use the same exercise more than twice in a single day. Spread the work across the day's assigned primary muscle groups rather than hammering one — no single muscle group should dominate the session unless the day's focus is explicitly that one area. Prefer distinct movements over padding the workout with repeats of the same exercise. **The one exception is a ramping set ladder**: when a strength block prescribes ONE lift at DIFFERENT loads (Wendler 5/3/1 warm-up sets then 65/75/85% working sets, a 5x5 work-up, working up to a heavy single), list EVERY set as its own entry with its own weight (sets: 1, reps as prescribed), in load order, inside a single \`traditional\` block — up to 8 entries for that lift. Never collapse a percentage scheme to two sets. Identical-load sets are still ONE entry with sets > 1. Loads: when the request states a 1RM, training max = 90% of THAT lift's 1RM (never reuse another day's number); round every barbell load UP to the nearest 5 lb; no set may exceed the stated 1RM.
...
7. **Named programs use their canonical implement**: when the user or the day's plan names a specific program or protocol, its main lifts use the implement that program is defined with, as long as the user's equipment includes it. Wendler 5/3/1 (also written "531") means BARBELL bench press, back squat, deadlift, and overhead press — never dumbbell substitutes when barbells are available. The same principle applies to any named program (e.g. Starting Strength, StrongLifts = barbell lifts; a named hero WOD keeps its prescribed movements). Substitute the implement ONLY when the required equipment is genuinely missing, and then say so in the day description.
...
- Use a variety of exercises: do NOT repeat the same exercise more than twice in this workout (sole exception: a ramping set ladder for one lift at distinct loads in one traditional block — list every set, up to 8), and distribute the work across this day's primary muscle groups
